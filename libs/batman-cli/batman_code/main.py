"""Main entry point and CLI loop for bat-code."""
...
from batman_code.integrations.sandbox_factory import create_sandbox
from batman_code.model_config import ModelConfigError
from batman_code.sessions import (
...
from batman_code.skills import execute_skills_command, setup_skills_parser
from batman_code.tools import fetch_url, http_request, web_search
...
def check_cli_dependencies() -> None:
    ...
    if missing:
        ...
        print("\nThe following packages are required to use bat-code:")
        for pkg in missing:
            print(f"  - {pkg}")
        print("\nPlease install them with:")
        print("  cd libs/batman-cli && uv sync")
        sys.exit(1)
...
def parse_args() -> argparse.Namespace:
    ...
    # `bat-code <subcommand> -h` shows context-specific help.
    ...
    parser = argparse.ArgumentParser(
        description=("bat-code — Batman-themed AI Coding Assistant"),
    ...
    threads_list.add_argument(
        "--persona", default=None, help="Filter by persona name (default: show all)"
    )
    threads_list.add_argument(
        "--limit",
        type=int,
    ...
    parser.add_argument(
        ...
        metavar="NAME",
        help=(
            "Persona to adopt: batman (default), alfred, oracle, "
            "nightwing, or joker. Sets both the prompt overlay and the "
            "memory namespace."
        ),
    )
    ...
    parser.add_argument(
        "--no-splash",
        action="store_true",
        help="Skip the Batcave loading animation on startup.",
    )
    ...
    parser.add_argument(
        ...
        "--version",
        action="version",
        version=f"bat-code {__version__}",
    )
    parser.add_argument(
        "-h",
        "--help",
...
async def run_textual_cli_async(
    ...
    *,
    ...
    Args:
        ...
        persona: Persona name for prompt overlay + joker startup modal.
        no_splash: Skip the Batcave splash on mount.

    Returns:
        The app's return code (0 for success, non-zero for error).
    """
    from batman_code.app import run_textual_app
...
def apply_stdin_pipe(args: argparse.Namespace) -> None:
    ...
    - If `non_interactive_message` is already set (`-n`), prepends the
        ...
        ```bash
        cat context.txt | bat-code -n "summarize this"
    ...
    - If `initial_prompt` is already set (`-m`, but not `-n`), prepends
        ...
        ```bash
        cat error.log | bat-code -m "explain this"
    ...
    - Otherwise, sets `non_interactive_message` to the piped text, causing
        ...
        ```bash
        echo "fix the typo in README.md" | bat-code
...
def cli_main() -> None:
    ...
    # BATCODE_LANGSMITH_PROJECT while shell commands use the
    ...
    try:
        ...
        if args.shell_allow_list:
            from batman_code.config import parse_shell_allow_list
        ...
        if args.default_model is not None:
            from batman_code.model_config import (
                ModelConfig,
                save_default_model,
            )
            ...
            from batman_code.config import detect_provider
            from batman_code.model_config import ModelSpec
        ...
        if args.command == "help":
            ...
            if args.threads_command in {"list", "ls"}:
                asyncio.run(
                    list_threads_command(
                        agent_name=getattr(args, "persona", None),
            ...
            from batman_code.non_interactive import run_non_interactive

            exit_code = asyncio.run(
                run_non_interactive(
                    message=args.non_interactive_message,
                    assistant_id=args.persona,
            ...
            if args.resume_thread == "__MOST_RECENT__":
                ...
                # If --persona specified, filter by that persona; otherwise get
                # most recent overall
                agent_filter = args.persona if args.persona != DEFAULT_PERSONA else None
                ...
                if asyncio.run(thread_exists(args.resume_thread)):
                    ...
                    is_resumed = True
                    if args.persona == DEFAULT_PERSONA:
                        agent_name = asyncio.run(get_thread_agent(thread_id))
                        if agent_name:
                            args.persona = agent_name
                else:
