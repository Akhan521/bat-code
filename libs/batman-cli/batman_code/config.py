"""Configuration, constants, and model creation for bat-code."""
...
_batcode_project = os.environ.get("BATCODE_LANGSMITH_PROJECT")
_original_langsmith_project = os.environ.get("LANGSMITH_PROJECT")
if _batcode_project:
    os.environ["LANGSMITH_PROJECT"] = _batcode_project
...
def _is_editable_install() -> bool:
    """Check if batman-cli is installed in editable mode.
...
# ── Interactive commands (Batman-themed) ────────────────────────────────
COMMANDS = {
    "clear": "Wipe the Batcomputer's screen",
    "help": "Access the Batcomputer's manual",
    "remember": "Update the Batcomputer's case files",
    "tokens": "Check the Batcomputer's resource usage",
    "quit": "Leave the Batcave",
    "exit": "Leave the Batcave",
}
...
def _find_project_agent_md(project_root: Path) -> list[Path]:
    ...
    1. project_root/.bat-code/AGENTS.md
    ...
    # Check .bat-code/AGENTS.md (preferred)
    batcode_md = project_root / ".bat-code" / "AGENTS.md"
    if batcode_md.exists():
        paths.append(batcode_md)
...
@dataclass
class Settings:
    """Global settings and environment detection for bat-code.
    ...
    Attributes:
        ...
        google_cloud_project: Google Cloud project ID for VertexAI
            authentication.
        batcode_langchain_project: LangSmith project name for bat-code
            agent tracing.
    ...
    batcode_langchain_project: str | None  # For bat-code agent tracing
    ...
    def from_environment(cls, *, start_path: Path | None = None) -> "Settings":
        ...
        batcode_langchain_project = os.environ.get("BATCODE_LANGSMITH_PROJECT")
        ...
        shell_allow_list_str = os.environ.get("BATCODE_SHELL_ALLOW_LIST")
        ...
        return cls(
            ...
            batcode_langchain_project=batcode_langchain_project,
    ...
    def user_deepagents_dir(self) -> Path:
        """Get the base user-level .bat-code directory.

        Returns:
            Path to ~/.bat-code
        """
        return Path.home() / ".bat-code"
    ...
    def get_user_agent_md_path(agent_name: str) -> Path:
        ...
        Returns:
            Path to ~/.bat-code/{agent_name}/AGENTS.md
        """
        return Path.home() / ".bat-code" / agent_name / "AGENTS.md"
    ...
    def get_project_agent_md_path(self) -> Path | None:
        ...
        Returns:
            Path to {project_root}/.bat-code/AGENTS.md, or None if not in a project
        """
        if not self.project_root:
            return None
        return self.project_root / ".bat-code" / "AGENTS.md"
    ...
    def get_agent_dir(self, agent_name: str) -> Path:
        ...
        return Path.home() / ".bat-code" / agent_name
    ...
    def get_user_skills_dir(self, agent_name: str) -> Path:
        ...
        Returns:
            Path to ~/.bat-code/{agent_name}/skills/
        """
    ...
    def get_project_skills_dir(self) -> Path | None:
        ...
        Returns:
            Path to {project_root}/.bat-code/skills/, or None if not in a project
        """
        if not self.project_root:
            return None
        return self.project_root / ".bat-code" / "skills"
    ...
    def get_user_agents_dir(self, agent_name: str) -> Path:
        ...
        Args:
            agent_name: Name of the CLI agent (e.g., "bat-code")

        Returns:
            Path to ~/.bat-code/{agent_name}/agents/
        """
    ...
    def get_project_agents_dir(self) -> Path | None:
        ...
        Returns:
            Path to {project_root}/.bat-code/agents/, or None if not in a project
        """
        if not self.project_root:
            return None
        return self.project_root / ".bat-code" / "agents"
    ...
    def get_built_in_skills_dir() -> Path:
        """Get the directory containing built-in skills that ship with bat-code.
...
def get_langsmith_project_name() -> str | None:
    ...
    `settings.batcode_langchain_project` (from
    `BATCODE_LANGSMITH_PROJECT`), then `LANGSMITH_PROJECT` from the
    ...
    return (
        settings.batcode_langchain_project
...
def build_langsmith_thread_url(thread_id: str) -> str | None:
    ...
    return f"{project_url.rstrip('/')}/t/{thread_id}?utm_source=bat-code"
...
def validate_model_capabilities(model: BaseChatModel, model_name: str) -> None:
    """Validate that the model has required capabilities for bat-code.
    ...
    if tool_calling is False:
        console.print(
            f"[bold red]Villain Detected:[/bold red] Model '{model_name}' "
            "does not support tool calling."
        )
        console.print(
            "\nbat-code requires tool calling for agent functionality. "
