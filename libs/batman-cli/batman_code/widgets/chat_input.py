"""Chat input widget for bat-code with autocomplete and history support."""
...
from batman_code.config import CharsetMode, _detect_charset_mode, get_glyphs
from batman_code.widgets.autocomplete import (
    SLASH_COMMANDS,
    CompletionResult,
    FuzzyFileController,
...
from batman_code.widgets.history import HistoryManager
...
class ChatInput(Vertical):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        Args:
            ...
            history_file: Path to history file (default: ~/.bat-code/history.jsonl)
            **kwargs: Additional arguments for parent
        """
        super().__init__(**kwargs)
        ...
        # Set up history manager — keeps chat history with the rest of
        # bat-code's data dir so we don't share state with deepagents-cli.
        if history_file is None:
            history_file = Path.home() / ".bat-code" / "history.jsonl"
    ...
    def on_mount(self) -> None:
        ...
        if _detect_charset_mode() == CharsetMode.ASCII:
            # Match the Unicode border which renders bat-gold via $primary.
