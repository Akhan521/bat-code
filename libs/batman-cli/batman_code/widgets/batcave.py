...
def _compose_crt_text(text: str) -> list[str]:
    """Compose text using 6-row CRT font (single word, no spaces)."""
...
_BOOT_MONOLOGUES: list[str] = [
    # -- Dramatic / Cinematic --
    "Every great case starts the same way \u2014 a dark room, a cold screen, and the will to see it through. The Batcomputer powers on and the investigation begins. Its agents deploy like detectives into the night, each one chasing a lead through the codebase. Gotham's greatest mystery isn't out there. It's in the code. Are you ready, detective?",
    "They told Bruce Wayne the cave was a tomb. They were wrong. It's a launchpad. Tonight the Batcomputer ignites and its agents scatter into the darkness \u2014 each one armed with purpose, each one hunting through the code like bats through the Gotham night. When the sun rises, every line will be cleaner than the last.",
    "The Bat-Signal doesn't just light up the sky. It wakes something beneath the earth. The Batcomputer surges online \u2014 deep agents deploying faster than Gotham can blink. They sweep the codebase the way the Dark Knight sweeps the streets. Thorough. Unforgiving. By the time the signal fades, the work is already done.",
    "There's a reason criminals fear the night. Somewhere beneath the streets, a machine thinks faster than any of them can run. The Batcomputer \u2014 cold, relentless, absolute. Its agents fan out like wings across the code, and where they pass, nothing broken survives. Gotham's true protector doesn't wear a badge. It runs on purpose.",
    "Beneath the streets of Gotham, a mind sharper than any blade comes online. The Batcomputer \u2014 forged in darkness, powered by purpose. Deep agents deploy like shadows across the codebase. No vulnerability survives the night. No bug outruns the Dark Knight. This is not just a machine. This is justice.",
    "They built firewalls. He built something better. Tonight the cave wakes \u2014 processors humming, agents spawning, each one carrying a piece of the mission. The codebase is the city. The bugs are the criminals. And the Dark Knight doesn't sleep until every last one is found.",
    "Most machines answer questions. This one interrogates them. The Batcomputer doesn't accept the first explanation \u2014 it digs deeper, cross-references, and corners the truth the way Batman corners a suspect in a dead-end alley. Its agents don't just scan code. They cross-examine it. Welcome back, detective. We have a case to solve.",
    "Wayne Tower touches the clouds. But the real power lives below \u2014 buried in stone, wrapped in silence, humming with intent. The Batcomputer awakens and Gotham holds its breath. Agents surge through the codebase like the Bat-Family through the rooftops. No corner unchecked. No shadow unturned. The Dark Knight's crusade doesn't end at dawn. It compiles.",
    "The cave shudders awake. Screens flare. Deep in the bedrock beneath Wayne Manor, the Batcomputer draws its first breath of the night \u2014 and with it, a swarm of agents rises. They pour through the codebase like shadows through Gotham's alleyways. The Dark Knight doesn't wait for problems to find him. He finds them first.",
...
def _build_computer_cells(
    ...
    # Match parity with screen_w so centering is symmetric
    if mw % 2 != screen_w % 2:
        mw -= 1
    ...
    # BATCOMPUTER title (6-row CRT font)
