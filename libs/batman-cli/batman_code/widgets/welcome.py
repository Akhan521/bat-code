"""Welcome banner widget for bat-code."""
...
# Gotham welcome message pool — one is picked at random per banner render.
_GOTHAM_WELCOMES: tuple[str, ...] = (
    "Gotham needs you.",
...
class WelcomeBanner(Static):
    ...
    def __init__(self, thread_id: str | None = None, **kwargs: Any) -> None:
        ...
        # Pin the welcome message at construction so re-renders (LangSmith URL
        # arrival, thread-ID updates) don't shuffle it.
        self._welcome_message: str = random.choice(_GOTHAM_WELCOMES)  # noqa: S311
    ...
    def _build_banner(self, project_url: str | None = None) -> Text:
        ...
        """
        banner = Text()
        # Use darker gold for editable installs, bat-gold for production
        banner_color = (
        ...
        if self._project_name:
            ...
            if project_url:
                banner.append(
                    ...
                    style=Style(
                        color="cyan",
                        link=f"{project_url}?utm_source=bat-code",
                    ),
                )
            else:
